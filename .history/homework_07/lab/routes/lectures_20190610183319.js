...
router.delete('/', async function(req, res) {
  let course = req.body;
  const result = let result = await databaseLecture(req).deleteOne(course);
  res.json({message : "saved", data : result});
});

router.get('/search/:q',async(req,res)=>{
  const result = let result = await databaseLecture(req).find({'lecture':{'$regex':req.params.q}}).toArray();
  res.json(result);
});
