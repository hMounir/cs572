...
router.get('/getAllSpecificFields4', async function(req, res) {
    await databaseLecture(req).find({}).project({district:'Bronx'}).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields5', async function(req, res) {
    await databaseLecture(req).find({}).project({district:'Bronx'}).limit(5).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields6', async function(req, res) {
    await databaseLecture(req).find({}).project({district:'Bronx'}).skip(5).limit(5).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields7', async function(req, res) {
    await databaseLecture(req).find().project({'address.coord': { $elemMatch: { $lt: -95.754168 } } }).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
