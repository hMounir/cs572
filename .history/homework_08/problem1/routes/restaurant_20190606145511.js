...
//10- Write a query to find the restaurant id, name, district and cusine
//for those restaurants which are not belonging to the district staten island 
// or Queens or Bronx or Brooklyn
router.get('/getAllSpecificFields8', async function(req, res) {
    let result = await databaseLecture(req).find({
        cuisine: { $ne: 'American' },
        'grades.score': { $gt: 70 },
