...
router.get('/getSpecificFieldsExceptId1', async function(req, res) {
    await databaseLecture(req).find({},{restaurant_id: 1,name: 1,district: 1,cuisine: 1}).limit(100).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFieldsExceptId3', async function(req, res) {
    await databaseLecture(req).find({},{ _id: 0,restaurant_id: 1,name: 1,district: 1,"address.zipcode": 1}).limit(100).toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
