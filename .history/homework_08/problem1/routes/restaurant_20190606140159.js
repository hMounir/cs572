...
router.get('/getSpecificFieldsExceptId1', async function(req, res) {
    let result = await databaseLecture(req).find({},{restaurant_id: 1,name: 1,district: 1,cuisine: 1}).limit(100).toArray();
    res.json({message : "saved", data : result});;
});
...
router.get('/getAllSpecificFieldsExceptId3', async function(req, res) {
    let result = await databaseLecture(req).find({},{ _id: 0,restaurant_id: 1,name: 1,district: 1,"address.zipcode": 1}).limit(100).toArray();
    res.json({message : "saved", data : result});;
});
