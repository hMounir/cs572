...
router.get('/getAll', async function(req, res) {
    let result = let result = await databaseLecture(req).find().toArray();
    res.json({message : "saved", data : result}); 
});

//2- 
router.get('/getAllSpecificFields1', async function(req, res) {
    let result = let result = await databaseLecture(req).find({},{ projection: {restaurant_id: 1,name: 1,district: 1,cuisine: 1}}).toArray();
    res.json({message : "saved", data : result});
});
