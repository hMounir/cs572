...
router.get('/getAllSpecificFields15', async function(req, res) {
    await databaseLecture(req).find({ 'grades.score': { $ne: {$lt:10} } }).project({ restaurant_id: 1, name: 1, district: 1, cuisine: 1 })
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields16', async function(req, res) {
    await databaseLecture(req).find({ 'address.coord.1': {$gt:42,$lte:52} }).project({ restaurant_id: 1, name: 1, address: 1, 'address.coord': 1 })
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields17', async function(req, res) {
    await databaseLecture(req).find({}).sort({'name':1})
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields19', async function(req, res) {
    await databaseLecture(req).find({}).sort([['cuisine',1],['district',-1]])
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields20', async function(req, res) {
    await databaseLecture(req).find({'coord':{$type:'double'}})
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
...
router.get('/getAllSpecificFields21', async function(req, res) {
    await databaseLecture(req).find({'coord':{$type:'double'}})
    .project({name:1,district:1,'address.coord.0':'longitude', 'address.coord.1':'latitude'})
    .toArray(function(err, docArray) { 
        res.json({message : "saved", data : docArray}); 
    });
});
